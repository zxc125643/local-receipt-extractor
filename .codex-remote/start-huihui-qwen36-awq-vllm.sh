...
exec /home/yi/vLLM-2080Ti-Definitive/.venv/bin/python -m vllm.entrypoints.openai.api_server \
  ...
  --model /home/yi/models/Huihui-Qwen3.6-27B-abliterated-AWQ-MTP \
  --served-model-name huihui-qwen36-27b-abliterated-awq-mtp \
  --dtype half --tensor-parallel-size 2 --generation-config auto \
  --max-model-len 131072 --enable-chunked-prefill \
  --max-num-seqs 1 --max-num-batched-tokens 2048 \
  --quantization awq_marlin --gpu-memory-utilization 0.86 \
