...
import { saveReceiptManual } from "../api/client";
...
export function ReceiptExtractorPage() {
  ...
  const [result, updateResult] = useState<{ job_id: string; columns: string[]; rows: Array<Record<string, string>> } | null>(null);
  ...
  const setResult = (next: typeof result) => {
    const saved = history.find((item) => item.job_id === next?.job_id);
    setManualEntries((saved?.manual_entries || []) as ManualEntry[]);
    setManualText(saved?.manual_draft || '');
    setManualSaveState('');
    setResultExpanded(Boolean(next));
    updateResult(next);
  };
  useEffect(() => {
    if (!result) return;
    const jobId = result.job_id;
    setHistory((items) => items.map((item) => item.job_id === jobId ? { ...item, manual_entries: manualEntries, manual_draft: manualText } : item));
    setManualSaveState('正在保存到当前批次…');
    saveQueue.current = saveQueue.current.catch(() => undefined).then(() => saveReceiptManual(jobId, manualEntries, manualText)).then(() => setManualSaveState('已保存到当前历史批次')).catch((error) => { setManualSaveState('保存失败'); setNotice({ tone: 'error', text: String(error) }); });
  }, [result?.job_id, manualEntries, manualText]);
  useEffect(() => {
    void getReceiptHistory().then(async (items) => {
      try {
        const legacyRaw = window.localStorage.getItem("receipt-manual-entries");
        const legacy = legacyRaw ? JSON.parse(legacyRaw) as ManualEntry[] : [];
        if (legacy.length && items[0] && !(items[0].manual_entries?.length)) {
          await saveReceiptManual(items[0].job_id, legacy, '');
          items[0] = { ...items[0], manual_entries: legacy, manual_draft: '' };
        }
        if (items[0]) window.localStorage.removeItem("receipt-manual-entries");
  ...
  const processMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setManualSaveState('');
      updateResult(data);
      void getReceiptHistory().then((items) => setHistory((currentItems) => {
        const optimistic = currentItems.find((item) => item.job_id === data.job_id);
        return items.map((item) => item.job_id === data.job_id ? {
          ...item,
          manual_entries: optimistic?.manual_entries || manualEntries,
          manual_draft: optimistic?.manual_draft ?? manualText,
        } : item);
      })).catch(() => undefined);
    ...
    <section className="page receipt-page">
      ...
      {result ? <div className="panel receipt-results">
        <div className="panel-header">
          ...
          <div className="receipt-actions"><button className="secondary-button" type="button" onClick={() => setResultExpanded((value) => !value)}>{resultExpanded ? '收起' : '展开'}</button><button className="primary-button" type="button" onClick={() => void downloadReceiptWorkbook({ ...result, title: reportTitle, manual_entries: effectiveManualEntries }).catch((error) => setNotice({ tone: "error", text: error instanceof Error ? error.message : "导出失败。" }))}>导出 Excel</button></div>
        </div>
        {resultExpanded ? <>{effectiveManualEntries.length ? <p className="muted-text">金额组成：识别记录 {result.rows.reduce((s, r) => s + (Number(r["付款金额"]) || 0), 0).toFixed(2)} + 手工补录 {effectiveManualEntries.reduce((s, r) => s + (Number(r.金额) || 0), 0).toFixed(2)} = 总计 {(result.rows.reduce((s, r) => s + (Number(r["付款金额"]) || 0), 0) + effectiveManualEntries.reduce((s, r) => s + (Number(r.金额) || 0), 0)).toFixed(2)}</p> : null}
