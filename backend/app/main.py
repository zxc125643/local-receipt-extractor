...
def _persist_job(job: ReceiptJob) -> None:
    with job.lock:
        ...
        payload = {
            "columns": job.columns, "rows": job.rows, "invoices": job.invoices,
            "files": [name for name, _ in job.files], "manual_entries": job.manual_entries,
            "manual_draft": job.manual_draft, "duplicate_count": job.duplicate_count,
...
def status(job_id: str) -> dict[str, object]:
    ...
    if job.status == "completed":
        result.update(columns=job.columns, rows=job.rows, invoices=job.invoices, title=job.title, manual_entries=job.manual_entries, manual_draft=job.manual_draft)
    if job.error:
...
@app.post("/api/manual/parse", dependencies=[Depends(_assert_auth)])
def parse_manual(payload: dict[str, object]) -> dict[str, object]:
    draft = payload.get("manual_draft", "")
    if not isinstance(draft, str):
        raise HTTPException(status_code=422, detail="补录文本格式无效。")
    entries, errors = parse_manual_draft(draft)
    return {"manual_entries": entries, "errors": errors}
